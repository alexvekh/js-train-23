...
class Basket {
  ...
  goods = []; // Створюємо новий пустий масив для зберігання товарів (goods) в кошику

  // Робимо метод addGood, що приймає один параметр - good, який потрібно додати до масиву
  addGood(good) {
    this.goods.push(good); // Додаємо новий товар в масив товарів
  }
  // Робимо метод calculateTotalPrice, що розраховує загальну вартість товарів в кошику з урахуванням знижки
  calculateTotalPrice() {
    const totalPrice = this.goods.reduce((acc, good) => acc + good.price, 0); // За допомогою метода reduce ми сумуємо вартість всіх товарів в масиві
    return this.discountPlan.applyDiscount(totalPrice);
    // переданий в конструктор об'єкт . метод об'єкта (дані для обчислення)
  }
