...
class TransferAdapter {
  ...
  constructor(transferSystem) {
    this.transferSystem = transferSystem; // Зберігаємо посилання на об'єкт WalletTransfer у властивості transferSystem
  }
  ...
  initiateTransfer(amount) {
    const calculatedAmount = this.calculateFee(amount); // Викликаємо допоміжний метод calculateFee для обчислення комісії за переказ та результат записуєм в константу calculatedAmount
    console.log(`Ініціюємо банківський переказ: $${calculatedAmount}`);
    return this.transferSystem.processTransfer(calculatedAmount);
  }

  // Викликаємо метод  об'єкту WalletTransfer з calculatedAmount.
  processTransfer(calculatedAmount) {
    return this.transferSystem.processTransfer(calculatedAmount);
  }
